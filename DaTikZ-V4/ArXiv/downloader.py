...
ARCHIVE_DIR = "ArXiv/arxiv_archives"
...
class BaseDownloader(ABC):
    def _make_filter(self, timestamp, exclude=list()):
        def _filter(item):
    ...
    @abstractmethod
    def _item_download(self, identifier, index, **kwargs):
    ...
    def download(self, lazy=False, cutoff=datetime.fromtimestamp(0), exclude=list(), **dl_kwargs):
        archives = sorted(filter(self._make_filter(cutoff, exclude), self.items), key=self._to_timestamp, reverse=True)
...
class ArchiveDownloader(BaseDownloader):
    ...
    def _item_download(self, identifier, index, pattern="*.tar", timeout=TIMEOUT_MAX, **_):
        item = ia.get_item(identifier, request_kwargs=dict(timeout=timeout))
        item.download(destdir=ARCHIVE_DIR, glob_pattern=pattern, item_index=index, timeout=timeout)
...
class S3Downloader(BaseDownloader):
    ...
    def items(self):
        if self.has_aws_access:
            paginator = self._s3resource.meta.client.get_paginator("list_objects_v2")
    ...
    def _item_download(self, identifier, index, **_):
        src_file, target_dir = f"{identifier}.tar", join(ARCHIVE_DIR, identifier)
        makedirs(target_dir, exist_ok=True)
        self._s3resource.meta.client.download_file(
            Bucket='arxiv',
